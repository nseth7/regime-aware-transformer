...
def main() -> None:
    ...
    logging.info(
        "Loaded %s from epoch %d (val_IC=%.4f)",
        args.model,
        ckpt["epoch"],
        ckpt.get("ic", float("nan")),
    )
    ...
    ic = ic_summary(df)
    logging.info(
        "Test IC: mean=%+.4f std=%.4f t=%+.2f (%d days)",
        ic["ic_mean"],
        ic["ic_std"],
        ic["ic_t_stat"],
        ic["n_days"],
    ...
    for (top_k, reb, cost), res in sorted(all_results.items()):
        print(
            f"{top_k:<8}{reb:<6}{cost:<11}{res['gross_sharpe']:>+8.3f}  "
    ...
    results_by_cost = {
        cost: res
        for (top_k, reb, cost), res in all_results.items()
        if top_k == bt_cfg.headline_top_k and reb == bt_cfg.headline_rebalance
    }
    plot_equity_curve(
        results_by_cost,
        os.path.join(args.out_dir, f"{args.model}_equity.png"),
        title=(
            f"{args.model} — Equity Curves (top_k={bt_cfg.headline_top_k}, reb={bt_cfg.headline_rebalance}d)"
        ),
    )
