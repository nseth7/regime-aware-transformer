...
def daily_ic(df: pd.DataFrame, pred_col: str = "pred", true_col: str = "y_cs") -> pd.Series:
    ...
    return (
        df.groupby("date")
        .apply(lambda g: g[pred_col].corr(g[true_col], method="spearman") if len(g) >= 10 else np.nan)
        .dropna()
    )
...
def run_backtest(
    ...
    return {
        ...
        "net_sharpe": sharpe(bt["net"]),
        "gross_t_stat": t_stat(bt["gross"]),
        "net_t_stat": t_stat(bt["net"]),
        ...
        "gross_max_dd": max_drawdown(bt["gross"]),
        "net_max_dd": max_drawdown(bt["net"]),
        "win_rate": (bt["gross"] > 0).mean(),
        "avg_turnover": bt["turnover"].mean(),
        "avg_cost_drag": bt["cost"].mean(),
        "bt": bt,
    }
