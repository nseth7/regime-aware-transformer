...
def plot_vs_benchmark(
    ...
    strat_eq = np.asarray(strat_equity.values, dtype=float)
    ...
    bench_dd_vals = np.asarray(bench_dd.values, dtype=float)
    ...
    axes[1].plot(bench_dd.index, bench_dd_vals * 100, linewidth=1.4, label=benchmark_label)
    axes[1].fill_between(bench_dd.index, bench_dd_vals * 100, 0, alpha=0.2)
