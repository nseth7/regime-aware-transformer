...
@torch.no_grad()
def predict_split(model, X: torch.Tensor, macro: torch.Tensor, device: torch.device, batch_size: int = 512) -> np.ndarray:
    preds = []
...
def build_prediction_frame(data_dir: str, model, device: torch.device, split: str = "test") -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        "date": pd.to_datetime(dates), "ticker": tickers,
        "pred": preds, "y_cs": y_cs, "y_raw": y_raw,
    })
